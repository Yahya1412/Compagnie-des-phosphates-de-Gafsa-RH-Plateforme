...
def ajouter_tache(request):
    ...
    if request.method == 'POST':
        ...
        if not description:
            messages.warning(request, 'Le champ Description est nécessaire')
            return render(request, 'taches/ajouter_tache.html', context)

        montant = request.POST['montant']
        if not montant:
            messages.warning(request, 'Le champ Besoins financier de la tâche est nécessaire')
            return render(request, 'taches/ajouter_tache.html', context)
        unite = request.POST['montant']
        if not montant:
            messages.warning(request, 'Le champ Unité de la tâche est nécessaire')
            return render(request, 'taches/ajouter_tache.html', context)

        etat = request.POST['etat']
        if not etat:
            messages.warning(request, 'Le champ Etat est nécessaire')
            return render(request, 'taches/ajouter_tache.html', context)

        date_debut = request.POST['date_debut']
        if not date_debut:
            messages.warning(request, 'Le champ Date Début est nécessaire')
            return render(request, 'taches/ajouter_tache.html', context)

        date_fin = request.POST['date_fin']
        if not date_fin:
            messages.warning(request, 'Le champ Date Fin est nécessaire')
            return render(request, 'taches/ajouter_tache.html', context)

        employe = request.POST['employe']
        if not employe:
            messages.warning(request, 'Le champ Employé est nécessaire')
            return render(request, 'taches/ajouter_tache.html', context)
        ...
        try:
            ...
            messages.warning(request, 'Le format de date est incorrect')
            return render(request, 'taches/ajouter_tache.html', context)

        if date_fin <= date_debut:
            messages.warning(request, 'La date de fin doit être supérieure à la date de début')
            return render(request, 'taches/ajouter_tache.html', context)

        Taches.objects.create(
            nom=nom,
        ...
        messages.success(request, 'Tache enregistrée avec succès')
...
def tache_edit(request, id):
    ...
    if request.method == 'POST':
        ...
        if datetime.strptime(date_fin, '%Y-%m-%d') <= datetime.strptime(date_debut, '%Y-%m-%d'):
            messages.warning(request, 'La date de fin doit être supérieure à la date de début')
            return render(request, 'taches/tache_edit.html', context)
